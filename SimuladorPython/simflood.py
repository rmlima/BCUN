...
def flood(G,initiator,mostrar,resourcenodes):
    LOG = mostrar
    nextround=[]
    history=[]
    result=[]
    ...
    nextround = vs
    transmitions+=1
    if LOG: print "Emissão=",totalhops    
    for v in vs:
            msg_p2p+= 1            
    ...
    if LOG: print "Total hops=",totalhops+hop_found
    
    result.append(hop-1)                    #Time Complexity
    result.append(msg_p2p+hop_found)        #Communication Complexity
    result.append(msg_found)        	    #Messages until found
    result.append(totalhops+hop_found)      #Number of HOPs to reach the most distant node
