...
if [ $# -ne 2 ]
then
	echo -e "Usage:  simul.sh  <cnodes> <iter> <proto>\n **** rml@fam.ulusiada.pt v1.1 ****"
	exit 1
else
...
do
	...
	python genTraffileGBC.py $CNODES $c
	
	SCENARIO=$DATASET"/cenario"$c
	
	ns gbcsearch.tcl -traf traffile -mov $SCENARIO

	cp $RESULTPATH/rmlsearch.tr $LOG/rmlsearch$PROTO_$c.tr
