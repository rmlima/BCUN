...
set val(nn) 		 10
set val(stop)          	3100              ;# simulation time
...
set tracefd	[open $val(out)$val(proto).tr w] 
$ns_ use-newtrace
$ns_ trace-all $tracefd

if {$val(usenam)==1} {
    set namtrace    [open $val(out)$val(proto).nam w]
...
for {set i 0} {$i < $val(nn) } {incr i} {
    ...
    set bcir_($i) [new Agent/BCIR]
    $node_($i) attach $bcir_($i) $val(port)
    $ns_ attach-agent $node_($i) $bcir_($i)
    ...
    $bcir_($i) log-target $T
}


#Set resources
#    if { [info exists resource]  } {
	$bcir_($val(resource)) "resource"
