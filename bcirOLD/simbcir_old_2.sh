...
if [ $LOG -ne 0 ]
then
if [ ! -d "../backup/LOG" ]; then mkdir ../backup/LOG; fi
rm ../backup/LOG/*
fi
...
if [ ! -d "/dev/shm/ramfs" ]; then
    mkdir /dev/shm/ramfs
    mkdir /dev/shm/ramfs/even
    mkdir /dev/shm/ramfs/odd
else
	rm /dev/shm/ramfs/even/*
...
#	for ((resource=$(( $1-1 )); resource<$1; resource++)) > /dev/null 2>&1
	...
	do
		if [ "$(( $resource % 2 ))" -eq 0 ] ; then
			RESULTPATH="/dev/shm/ramfs/even"
			sed -i 's/set val(out).*/set val(out) \t\t "\/dev\/shm\/ramfs\/even\/rmlsearch"/g' rmlsearch.tcl
		else
...
echo "DelayFlood: rng.uniform(0.00001,$JITTER)"
