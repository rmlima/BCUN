...
#	for ((resource=$(( $NODES-1 )); resource<$NODES; resource++)) > /dev/null 2>&1
	...
	do
		...
		sem -j 100% ns rmlsearch.tcl -traf traffileflood -mov cenario -proto flood -resource $resource  > /dev/null 2>&1 &
		sem -j 100% ns rmlsearch.tcl -traf traffilebers -mov cenario -proto bers -resource $resource  > /dev/null 2>&1 &
		sem -j 100% ns rmlsearch.tcl -traf traffilebcir -mov cenario -proto bcir -resource $resource  > /dev/null 2>&1 &
