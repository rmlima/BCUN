...
float GbcAgent::bloomSearch(char *elem)
{
int j,idx;
float result;
unsigned char chave[KEYSIZE];
...
for (j=0; j<BLOOM_K; j++)
	...
	//printf("bloomSearch: idx=%d bloomSnt=%f\n",idx,bloomSnt[idx]);
	}

return (result);
}

float GbcAgent::bloomSearchRcv(char *elem)
{
int j,idx;
float result;
unsigned char chave[KEYSIZE];
...
for (j=0; j<BLOOM_K; j++)
	{
	idx=chave[j];
	result=min(result,bloomRcv[idx]);
	//printf("bloomSearch: idx=%d bloomRcv=%f\n",idx,bloomRcv[idx]);
	}
...
for (j=0 ; j<BLOOM_M; j++)
   {
   bloomSnt[j]=MAX(bloomSnt[j],bloomRcv[j]*att);
   }
}
...
char * GbcAgent::decimal_to_binary(int n)
{
   int c, d, count;
   char *pointer;
...
float forward[M]; //Adição da informação própria ao gradiente existente
